...
import BreakDownTicketsDatatable from "./BreakDownTicketsDataTable";
import BreakDownTicketsEditDialogComponent from "./BreakDownTicketsEditDialogComponent";
import BreakDownTicketsCreateDialogComponent from "./BreakDownTicketsCreateDialogComponent";
import BreakDownTicketsFakerDialogComponent from "./BreakDownTicketsFakerDialogComponent";
import BreakDownTicketsSeederDialogComponent from "./BreakDownTicketsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const BreakDownTicketsPage = (props) => {
    ...
    const filename = "breakDownTickets.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("breakDownTickets");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , checkListFormId : urlParams.singleCheckListFormsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Break Down Tickets", type: "error", message: error.message || "Failed get Break Down Tickets" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("breakDownTickets").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Break Down Tickets", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("breakDownTickets").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Break Down Tickets",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/breakDownTickets/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Break Down Tickets </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="breakDownTickets-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="breakDownTickets-datatable">
                    <BreakDownTicketsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <BreakDownTicketsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <BreakDownTicketsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <BreakDownTicketsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <BreakDownTicketsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(BreakDownTicketsPage);
