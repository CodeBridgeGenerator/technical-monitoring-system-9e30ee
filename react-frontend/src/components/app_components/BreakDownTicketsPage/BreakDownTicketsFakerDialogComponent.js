...
import breakDownTicketsFakerFactory from "./breakDownTicketsFakerFactory";
...
const BreakDownTicketsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [checkListFormIdIds, setcheckListFormIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("checkListForms")
                        .find({ query: { $select: ["_id","description"] } })
                        .then((data) => {
                        setcheckListFormIdIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = breakDownTicketsFakerFactory(props.user,fakerCount,checkListFormIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("breakDownTickets").create(elem));
...
export default connect(mapState, mapDispatch)(BreakDownTicketsFakerDialogComponent);
