...
import timerServicesFakerFactory from "./timerServicesFakerFactory";
...
const TimerServicesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = timerServicesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("timerServices").create(elem));
...
export default connect(mapState, mapDispatch)(TimerServicesFakerDialogComponent);
