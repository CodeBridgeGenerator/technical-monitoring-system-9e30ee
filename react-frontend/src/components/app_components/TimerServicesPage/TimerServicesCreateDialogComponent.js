...
const TimerServicesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            serviceName: _entity?.serviceName,startTime: _entity?.startTime,endTime: _entity?.endTime,status: _entity?.status,
        ...
        const result = await client.service("timerServices").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info TimerServices created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in TimerServices" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create TimerServices" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="timerServices-create-dialog-component">
