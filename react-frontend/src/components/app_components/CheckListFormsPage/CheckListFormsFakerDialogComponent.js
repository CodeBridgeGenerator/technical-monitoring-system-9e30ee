...
import checkListFormsFakerFactory from "./checkListFormsFakerFactory";
...
const CheckListFormsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = checkListFormsFakerFactory(props.user,fakerCount,checklistIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("checkListForms").create(elem));
...
export default connect(mapState, mapDispatch)(CheckListFormsFakerDialogComponent);
