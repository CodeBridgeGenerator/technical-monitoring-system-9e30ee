...
import CheckListFormsDatatable from "./CheckListFormsDataTable";
import CheckListFormsEditDialogComponent from "./CheckListFormsEditDialogComponent";
import CheckListFormsCreateDialogComponent from "./CheckListFormsCreateDialogComponent";
import CheckListFormsFakerDialogComponent from "./CheckListFormsFakerDialogComponent";
import CheckListFormsSeederDialogComponent from "./CheckListFormsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const CheckListFormsPage = (props) => {
    ...
    const filename = "checkListForms.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("checkListForms");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , checklistId : urlParams.singleCheckListsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("checkListForms").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "CheckListForms", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("checkListForms").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "CheckListForms",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/checkListForms/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="checkListForms-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="checkListForms-datatable">
                    <CheckListFormsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CheckListFormsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CheckListFormsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CheckListFormsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CheckListFormsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CheckListFormsPage);
