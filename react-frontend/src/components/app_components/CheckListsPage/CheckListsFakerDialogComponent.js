...
import checkListsFakerFactory from "./checkListsFakerFactory";
...
const CheckListsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = checkListsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("checkLists").create(elem));
...
export default connect(mapState, mapDispatch)(CheckListsFakerDialogComponent);
