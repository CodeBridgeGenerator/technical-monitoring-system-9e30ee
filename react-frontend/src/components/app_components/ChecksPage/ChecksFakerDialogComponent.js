...
import checksFakerFactory from "./checksFakerFactory";
...
const ChecksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = checksFakerFactory(props.user,fakerCount,checkListIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("checks").create(elem));
...
export default connect(mapState, mapDispatch)(ChecksFakerDialogComponent);
